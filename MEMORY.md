...
### 🎨 Marketing Studio — cockpit de marketing com IA (decidido 2026-06-03)
Bruno quer uma **plataforma web acessível por link, de qualquer computador**, pra produzir o marketing. Visão dele (= um "estúdio de marketing com IA"): planejamento de campanha + conteúdo orgânico (ligados, em **JSON**); área de criação que "puxa o plano" e gera copy/imagem/carrossel; seção de **vídeo** (roteiro→JSON→render, ideal via **Remotion**; AE/Photoshop/Premiere por "vibecoding" = frágil, fora do core); **ads pagos** (Meta/TikTok/LinkedIn/Google — gerar criativos/textos/persona/horário/orçamento; publicar: Meta/Google via API dá, TikTok/LinkedIn = brief+manual); área de prompt livre. NÃO é treinar LLM — é **orquestração (agente) plugando IAs prontas** (Claude + geradores). O **"plano em JSON" é a espinha** que liga tudo.
- **DECISÃO (Bruno escolheu)**: **cockpit ENXUTO INTERNO primeiro** — Bruno usa no marketing do Vetly (dogfood), modular; se validar, vira **Produto #2** (mercado diferente: marketeiros/agências/founders; espaço concorrido: Jasper/AdCreative/Predis/Creatify). De-risca + resolve a dor agora sem tirar foco do Vetly (launch 22/07).
- **MVP proposto (a confirmar)**: app Next.js separado, por link; Módulo 1 Planejamento (Claude→JSON) + Módulo 2 Criação (copy + carrossel via Gamma). Vídeo (Remotion, aí Docker entra de verdade no render) e ads = fase 2. Material-fonte já existe: `BRUNO/marketing/vetly-campaign-plan-v1.md` + `vetly-boilerplate-press-kit.md`.
- **Status**: plano do MVP sendo montado; aguardando Bruno confirmar escopo + decisões (hosting Vercel-free vs VPS, provedor de imagem/orçamento, contas/keys, nome).
