...
## 🐛 PRIMEIRO FEEDBACK DE CLIENTE REAL (2026-06-03) — começou a iteração técnica
Primeiro cliente potencial (construtora, testou proposta de esquadrias). **Amou** a análise e a apresentação. Reportou 3 problemas:

- **BUG 1 — RESOLVIDO E DEPLOYADO (commit df6bd8e).** `/configuracoes` dava "Application error: client-side exception" ao aplicar logo/cores → não conseguia subir arquivos. **Causa-raiz**: formulários com `useFormState` liam `estado.erro`/`estado.sucesso` sem guarda; quando o POST da action falha (413/rede/erro server), `estado` vira `undefined` e ler `.erro` lança exceção no cliente = tela branca. **Mesmo bug já tinha acontecido antes no PropostaForm.** Correção definitiva ("NUNCA MAIS", ordem do Bruno): blindei `estado?.erro`/`estado?.sucesso` em **TODOS** os forms (configuracoes, comparativos, fornecedores, auth, aprovação pública). Padrão a seguir sempre: **todo `useFormState` lê estado com `?.`**.
- **BUG 2 — RESOLVIDO E DEPLOYADO (commits 833313e + 41f4783).** Aprovar uma **concorrência (comparativo) pelo link** só marcava `apresentada`, sem marcar nenhuma proposta como aprovada. **Regra de produto (Bruno):** "uma proposta pode ser individual ou em concorrência; se aprovada em concorrência, precisa virar aprovada nos dois cenários." Implementado SEM migration (reusa `comparativos.proposta_escolhida_id`):
  - **Comprador recomenda (sugestão)**: ao Compartilhar/enviar e-mail de um comparativo, escolhe a proposta recomendada (`recomendarProposta` em `lib/comparativos/db.ts` — seta escolhida + `apresentada`, NÃO aprova).
  - **Aprovador decide (refinamento do Bruno: "quem aprova escolhe"):** na página pública `/r/[token]`, o `FormAprovacao` mostra as propostas da concorrência como radios, com a **recomendada pré-marcada** (badge "Recomendada"); o aprovador pode manter ou trocar. Campo `proposta_aprovada_id` (schema, opcional). `registrarAprovacao` usa a escolha DO APROVADOR (fallback p/ a recomendada): vencedora → `aprovada`, demais → `recusada`, comparativo → `decidida`, e grava a vencedora em `proposta_escolhida_id`. Recusado/sem escolha → só `apresentada`. Banner público "Recomendação" mostra a escolha do comprador.
- **BUG 3 (era falso alarme)** — filtro/indicador "Aprovadas". O dropdown de situação em /propostas SEMPRE teve "Aprovada"; o **chip "Aprovadas" do painel é admin-only** (`{admin && ...}`), e o dono do workspace JÁ é admin (RLS prova: ele vê as propostas → tem `workspace_members` ativo, e dono entra como `role='admin'`). Ou seja, ele via, mas estava tudo `em_aberto` por causa do BUG 2. **Brecha latente anotada (não era a causa dele)**: `encontrarOuCriarUsuarioOAuth` (login Google) retorna cedo se o usuário já existe, sem garantir `workspace_members` — corrigir um dia para auto-heal.
...
- ✅ **Onboarding 1º acesso** — `components/painel/OnboardingNovo.tsx`: card "Como funciona" (4 passos) no painel, aparece só quando a conta está vazia. + `/ajuda/guia` (doc passo a passo de 10 passos, linkada da Ajuda).
- ✅ **Tour guiado interativo (2026-06-02, pedido do Bruno — opção B)** — `components/onboarding/TourGuia.tsx`: popups/spotlight ancorados em elementos (`data-tour`), atravessam páginas (nav Propostas → botão Nova proposta → dropzone → botão Analisar), com "Pular". Montado no layout (persiste na navegação), gate por `localStorage["vetly_tour_ok"]` (mostra 1x). NÃO cria nada nem gasta cota — só guia até a 1ª proposta real do usuário (opção B, não a A de proposta-amostra). Custom, SEM biblioteca. Re-disparar p/ demo: `localStorage.removeItem('vetly_tour_ok')` + refresh.
...
6. **i18n — inglês + espanhol (vender fora do Brasil)** — Bruno quer vender internacionalmente; o app precisa de pelo menos EN + ES. **Entra MAIS À FRENTE** (Bruno confirmou 2026-06-03: guardar plano, aplicar no momento certo; agora foco é venda/marketing). **PLANO AVALIADO E APROVADO (viável):**
   - **Detecção automática** pelo idioma do navegador (`Accept-Language`) no middleware → roteia pro locale certo na 1ª visita (melhor que GeoIP por país; hispano-falante nos EUA cai em ES). **Seletor manual** PT/EN/ES (topo/rodapé) + **cookie** lembra a escolha.
   - **Rotas por idioma** (`/en/...`, `/es/...`, PT padrão). Lib recomendada: **`next-intl`** (precisa de ADR — dependência nova).
   - **5 frentes**: (1) encanamento (rotas+detecção+seletor) ~1-2d; (2) traduzir TODA a UI (mover strings PT→arquivos pt/en/es: landing, painel, forms, erros, FAQ, tour, e-mails) — é o grosso; (3) **relatórios da IA no idioma** — passar locale aos prompts (`lib/ai/`) p/ Claude gerar análise/comparação/apresentação em EN/ES + revisar qualidade de termos de procurement + labels de PPT/PDF; (4) e-mails (Resend) + **moeda internacional** (decidir USD/EUR = novos preços no Stripe; checkout Stripe já se localiza); (5) QA dos 3 idiomas.
   - **Tempo**: enxuto (landing + fluxo core + relatório IA) **~4-6 dias**; completo **~8-13 dias** (≈2-2,5 semanas). Encanamento é rápido; o custo é volume de tradução + qualidade do relatório da IA em outra língua. ⚠️ i18n deixa cada feature futura ~3× mais lenta (tudo em 3 línguas) → fazer com tração/leads internacionais concretos.
   - **Decisões do Bruno quando for**: moeda (R$ vs USD/EUR), quem revisa traduções (fluente EN/ES, sobretudo os termos do relatório), escopo (enxuto primeiro vs tudo).
7. **Painel administrativo do GetVetly (interno, só pro Bruno)** — back-office do dono pra acompanhar **vendas/MRR, dashboards, conteúdos, site, blog** etc. Separado do app do cliente (área `/admin` protegida só pro Bruno, ou app à parte). **Entra quando o produto estiver finalizado para venda** (depende de Stripe pros dados de venda + métricas de uso já existentes em `workspace_uso`). ✅ JÁ FEITO (ver `/admin`).

> **Marketing Studio (cockpit de marketing com IA)** — ideia do Bruno, **movida para PROJETO SEPARADO** (2026-06-04) pra não misturar com o Vetly. Plano completo em `BRUNO/marketing-studio-plano.md` (fora do Git). **Não construir aqui.**
...
- **2026-08-21 — HOTFIX: análise longa cortada (commit ce3b934)** ✅ deployado e validado
  - **Incidente com cliente real (workspace "Selo7")**: subiu uma concorrência com 3 fornecedores (Mega/GKS/M2A) numa proposta só → resposta da IA estourou `max_tokens: 8192` (`stop_reason=max_tokens`) → análise falhava com "A resposta da IA foi cortada". Falhou 3x seguidas; a tela ficava em "Analisando" até o `failed` chegar (poller ok, mecanismo de erro funcionou).
  - **Correção**: `lib/ai/analisar.ts` + `lib/ai/comparar.ts` → `client.messages.stream(...).finalMessage()` com `max_tokens: 32000`. **Streaming é obrigatório** p/ respostas longas (SDK recusa non-streaming que estime >10 min). `deck-plan` (4096) e `design-tokens` (300) não mexi — outputs pequenos.
  - **Diagnóstico via SSH root@2.25.147.198** (chave deste PC funciona como root; como `deploy` NÃO): logs em `/home/deploy/.pm2/logs/getvetly-*.log`; psql com `source /var/www/getvetly/.env` + `$DATABASE_URL_SERVICE`. Coluna de erro da proposta = `status_message` (não "erro_mensagem").
  - ⚠️ Observado nos logs (não tratado ainda): PM2 com 149 restarts em ~2 meses (`ELIFECYCLE Command failed` recorrente) — investigar causa (possível OOM); e havia erro antigo de Resend 403 "domain not verified" no log (histórico).

- **2026-08-21 — Teste estendido pelo admin (commit 9fee998)** ✅ deployado
  - Pedido do Bruno: dar mais margem de teste p/ clientes estratégicos. **Trial não tem prazo em dias** — é medido em análises grátis (3), então a alavanca é **análises extras por workspace**.
  - Migration **0010** (`workspaces.analises_gratis_extra`, default 0, aplicada no VPS via `sudo -u postgres psql -d getvetly`). Limite grátis = `limiteAnalisesGratis(extra)` em `lib/stripe/config.ts` (= 3 + extra). `verificarAcessoAnalise` agora retorna `limite`.
  - **/admin → "Empresas recentes"**: coluna "Teste grátis" mostra `usadas/3+` + campo numérico p/ definir o extra (form server-action `definirAnalisesExtrasAction`, gated `ehEmailInterno`, Zod). Empresas `active` mostram "—".
  - **Detalhe importante**: análise que FALHA não consome cota (só conta linha em `analises`, gravada no sucesso). Cliente VirtuGnl (tiago.massa@virtugnl.com) tinha 0 usadas — não precisou de reset, só reanalizar após o hotfix do max_tokens.
  - Workspace **"Selo7"** = workspace de teste do PRÓPRIO Bruno (membro brunobrm@gmail.com, admin interno → ilimitado); "Selo7" é o nome whitelabel, não confundir com cliente.
