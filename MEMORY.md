...
- **2026-05-30 — fix: upload de proposta grande dava tela branca** ✅
  ...
  - Correção infra (manual no VPS): adicionado `client_max_body_size 30M;` no bloco `server` do Nginx do getvetly + `nginx -t` + `systemctl reload nginx`. **LIÇÃO:** uploads grandes precisam de DOIS limites — `serverActions.bodySizeLimit` (Next) E `client_max_body_size` (Nginx). Registrar pra não esquecer em features futuras de upload.
