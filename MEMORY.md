...
- **BUG 2 — RESOLVIDO E DEPLOYADO (commit 833313e).** Aprovar uma **concorrência (comparativo) pelo link** só marcava `apresentada`, sem marcar nenhuma proposta como aprovada. **Decisão de produto do Bruno** ("uma proposta pode ser individual ou em concorrência; se aprovada em concorrência, precisa virar aprovada nos dois cenários"; e ele escolheu **"o comprador marca a recomendada ANTES de enviar"**). Implementado SEM migration (reusa `comparativos.proposta_escolhida_id`): no Compartilhar/enviar e-mail de um comparativo, o comprador escolhe a proposta recomendada (`recomendarProposta` em `lib/comparativos/db.ts` — seta escolhida + `apresentada`, NÃO aprova ainda). Quando o aprovador aprova pelo link, `registrarAprovacao` faz a cascata: recomendada → `aprovada`, demais da concorrência → `recusada`, comparativo → `decidida` (espelha `decidirComparativo`). O banner público "Recomendação" mostra a escolha do comprador (não só a da IA). Recusado ou sem recomendada = comportamento antigo (só `apresentada`).
