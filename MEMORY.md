...
- **BUG 2 — EM DIAGNÓSTICO.** Ele "aprovou" mas não entrou em "Aprovadas" (painel + filtro seguem "Em aberto"). Código verificado: aprovar PROPOSTA (link público OU in-app via `atualizarSituacaoProposta`) marca `aprovada` corretamente. O ÚNICO caminho que não cascateia: aprovar um **COMPARATIVO** pelo link → só vira `apresentada`, NÃO marca proposta (by design: vencedora IA ≠ aprovada, decisão é manual no app). Hipótese forte: ele aprovou um comparativo. Falta confirmar com dados reais → criado `db/ops/diagnostico-e-reset-cliente.sql`.
- **BUG 3 (relacionado ao 2)** — filtro/indicador "Aprovadas" "não aparece". O dropdown de situação SEMPRE tem "Aprovada"; já o **chip "Aprovadas" do painel é admin-only** (`{admin && ...}` em painel/page.tsx). Dono do workspace deveria ser admin (cadastro insere `role='admin'`). **Atenção a uma brecha latente**: `encontrarOuCriarUsuarioOAuth` (login Google) retorna cedo se o usuário já existe, SEM garantir `workspace_members` — usuário antigo sem membership ficaria sem papel/sem painel (mas também não veria propostas pela RLS). Diagnóstico confirma o papel real dele.

**Ferramenta nova**: `db/ops/diagnostico-e-reset-cliente.sql` — roda no psql do VPS, escopado por e-mail. Parte 1 = diagnóstico (papel, propostas/situações, comparativos, shares, aprovações). Parte 2 = reset dos dados de teste (apaga conteúdo, mantém conta/workspace/papel; dry-run com `rollback`, trocar p/ `commit`).
