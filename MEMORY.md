...
- **E-mail @getvetly.com** — 3 contas (`bruno@`, `financeiro@`, `contato@`): **R$ 35,97** (criado 2026-06-01 — confirmar se é mensal ou anual). `contato@getvetly.com` agora existe → usado na landing.
...
- 🚧 **Stripe — config em andamento (2026-06-01)**: Bruno criou os 4 produtos no Stripe. **Product IDs**: Starter `prod_Ucy8s90IJb48ZB` · Pro `prod_Ucy9wxBKHD3mN9` · Business `prod_Ucy9PrSte6Mm62` · Enterprise `prod_UcyEcu6IreHiqN`. ⚠️ **FALTA (pedido ao Bruno)**: os **Price IDs** (`price_...` — o preço recorrente mensal DENTRO de cada produto; é o que o Stripe Checkout usa, NÃO o `prod_`); e as **API keys** (`sk_`/`pk_`, começar em **Test mode**) + `whsec_` do webhook → vão no `.env` do VPS (nunca no repo; IDs de produto/preço não são segredo, podem ficar aqui). Rota webhook planejada: `/api/webhooks/stripe`. Início de uso decidido: **teste grátis SEM cartão** (N análises grátis no cadastro → paywall pra continuar).
